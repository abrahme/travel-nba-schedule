...
#### Effect on Game Play
Here I'll conduct an analysis of how the schedule affects how we perceive team strength. To do so, I'll build a mixed effects model that estimates the chances of $team_i$ beating $team_j$.
\[
y_{ijt} = (\theta_i + \alpha_{i} home_{it}) - (\theta_j + \alpha_{j} home_{jt})
\]
where $y_{ijt}$ is the margin of victory of $team_i$ over $team_j$ at time $t$.
We can interpret the parameter $\theta_i$ as the *skill* of a team, as it relates to the team's average effect on their game's margin. Similarly, we can interpret the parameter $\alpha$ as the team's average benefit from playing at home.
The full details of the model are included in the `model.stan` file. The model borrows methodology from [here](`http://doogan.us/nj/NCAA2018.html`).
